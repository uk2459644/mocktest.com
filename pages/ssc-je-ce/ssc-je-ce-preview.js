...
const fetchPrevTestList = async () => await Axios.get('https://backend-mock-test-crash.herokuapp.com/previous-year-ssc-je-ce/preview/')
    .then(res => ({
        error: false,
...
const fetchTestList = async () => await Axios.get('https://backend-mock-test-crash.herokuapp.com/ssc-je-ce/preview/')
    .then(res => ({
        error: false,
        testlist: res.data,
    }
...
export default function SSCJECEPrivew({ prev_testlist, testlist }){
                                        ...
                                        testlist.testlist.map(test => (
                                            <div>
                                                <span class="flex shadow-md mb-5 text-xs m-4">
                                                    <Link href={`/ssc-je-ce/preview/${test.keyword}/${test.id}`}>
