...
const fetchPrevTestList = async () => await Axios.get('https://mocktestdjango-production.herokuapp.com/previous-year-ssc-je-ee/')
    .then(res => ({
        error: false,
...
const fetchTestList = async () => await Axios.get('https://mocktestdjango-production.herokuapp.com/ssc-je-ee/')
    .then(res => ({
        error: false,
        testlist: res.data,
    }
...
export default function SSCJEEEIndex({ prev_testlist, testlist }){
                                        ...
                                        testlist.testlist.map(test => (
                                            <div>
                                                <span class="flex shadow-md mb-5 text-xs m-4">
                                                    <Link href={`/ssc-je-ee/${test.keyword}/${test.id}`}>
