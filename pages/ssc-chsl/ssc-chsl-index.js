...
const fetchPrevTestList = async () => await Axios.get('https://mocktestdjango-production.herokuapp.com/previous-year-ssc-chsl/')
    .then(res => ({
        error: false,
...
const fetchTestList = async () => await Axios.get('https://mocktestdjango-production.herokuapp.com/ssc-chsl-testlist/')
    .then(res => ({
        error: false,
        testlist: res.data,
    }
...
export default function SSCCHSLIndex({ prev_testlist, testlist }){
                                        ...
                                        testlist.testlist.map(test => (
                                            <div>
                                                <span class="flex shadow-md mb-5 text-xs m-4">
                                                    <Link href={`/ssc-chsl/${test.keyword}/${test.id}`}>
