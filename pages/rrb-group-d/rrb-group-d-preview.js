...
const fetchPrevTestList = async () => await Axios.get('https://backend-mock-test-crash.herokuapp.com/previous-year-rrb-group-d/preview/')
    .then(res => ({
        error: false,
...
const fetchTestList = async () => await Axios.get('https://backend-mock-test-crash.herokuapp.com/rrb-group-d/preview')
    .then(res => ({
        error: false,
        testlist: res.data,
    }
...
export default function RRBGROUPDPrivew({ prev_testlist, testlist }) {
                ...
                <div className="container mx-auto py-4">
                    ...
                    <nav className="flex">
                        <button onClick={() => {
                            setLetTest(true)
                            setPrevTest(false)
                        }} className={` no-underline text-white py-2 px-2 font-medium mr-2 ml-4 rounded-sm bg-cyan-600 hover:bg-cyan-900`} >
                            Latest tests
                        </button>
                        <button onClick={() => {
                            setLetTest(false)
                            setPrevTest(true)
                        }} className={`no-underline text-white py-2 px-2 font-medium mx-2 rounded-sm bg-gray-900 hover:bg-gray-600 `} >
                            Previous year tests
                        </button>
                                        ...
                                        testlist.testlist.map(test => (
                                            <div>
                                                <span class="flex shadow-md mb-5 text-xs m-4">
                                                    <Link href={`/rrb-group-d/preview/${test.keyword}/${test.id}`}>
