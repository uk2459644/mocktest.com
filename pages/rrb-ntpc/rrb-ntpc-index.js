...
const fetchPrevTestList = async () => await Axios.get('https://mocktestdjango-production.herokuapp.com/previous-year-rrb-ntpc/')
    .then(res => ({
        error: false,
...
const fetchTestList = async () => await Axios.get('https://mocktestdjango-production.herokuapp.com/rrb-ntpc/')
    .then(res => ({
        error: false,
        testlist: res.data,
    }
...
export default function RRBNTPCIndex({ prev_testlist, testlist }){
                                        ...
                                        testlist.testlist.map(test => (
                                            <div>
                                                <span class="flex shadow-md mb-5 text-xs m-4">
                                                    <Link href={`/rrb-ntpc/${test.keyword}/${test.id}`}>
