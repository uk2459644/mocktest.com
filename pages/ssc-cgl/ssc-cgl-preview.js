...
const fetchPrevTestList = async () => await Axios.get('https://backend-mock-test-crash.herokuapp.com/previous-year-ssc-cgl/preview/')
    .then(res => ({
        error: false,
...
const fetchTestList = async () => await Axios.get('https://backend-mock-test-crash.herokuapp.com/ssc-cgl-testlist/preview/')
    .then(res => ({
        error: false,
        testlist: res.data,
    }
...
export default function SSCCGLPreview({testlist,prev_testlist}){

    //const {testlist} = useSWR('api/testlist',fetchTestList,{initialData:testlist});
                                        ...
                                        testlist.testlist.map(test => (
                                            <div>
                                                <span class="flex shadow-md mb-5 text-xs m-4">
                                                    <Link href={`/ssc-cgl/preview/${test.keyword}/${test.id}`}>
